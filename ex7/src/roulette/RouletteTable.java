...
public class RouletteTable {
	...
	public void placeBet(Player p,Field field, int value) throws NotEnoughChipsException, TooManyChipsException, TableFullException, NoMoreBetsException {
		placeBet(p,new Field[]{field}, value);
	}
	public void placeBet(Player p, Field fields[], int value) throws NotEnoughChipsException, TooManyChipsException, TableFullException, NoMoreBetsException {
		// validate bet
				...
				for (Field f: betsByFields.keySet()){
					if (f.winningStrategy().winsOn(ballPosition)){
						...
						for (Bet b:betsByFields.get(f)){
							walletService.adjustBalance(b.getPlayer(),b.getValue()*f.getPayoutCoeficient()/b.getNumberOfFields());
						}
					}
				}
			}
